...
Requested visual scope: selectores de moneda/idioma del header global y contenido expandido de cada trayecto, incluyendo validación de fechas, alineación de conceptos y editor de gastos sin cajas visibles, solicitado explícitamente por el product owner
...
- El formulario de gastos reutiliza el objeto `segment.expenses`; reordenar la presentación, alinear sus controles o agregar una fila de `others` no introduce llamadas de red, provider requests ni escrituras fuera del autosave/sync incremental ya existente.
