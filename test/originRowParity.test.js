...
test('la ciudad origen comparte la composición compacta de identidad, costo y acciones con los destinos', async () => {
  ...
  assert.match(origin, /className="itinerary-card__identity"/);
  ...
  assert.match(origin, /className="itinerary-card__side"/);
  ...
  assert.match(header, /className="itinerary-card__identity"/);
  assert.match(header, /className="itinerary-card__date itinerary-stop__date-range"/);
  assert.match(header, /className="itinerary-card__side"/);
  ...
  assert.match(compactCss, /grid-template-areas:\s*'visual identity side'\s*!important;/s);
  assert.match(compactCss, /\.itinerary-card__identity\s*\{[\s\S]*gap:\s*0\s*!important;/s);
  assert.match(compactCss, /\.itinerary-card__actions\s*\{[\s\S]*justify-content:\s*flex-end\s*!important;/s);
});
