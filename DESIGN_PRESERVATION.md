...
Requested visual scope: selectores de moneda/idioma del header global y contenido expandido de cada trayecto, incluyendo validación de fechas, alineación exacta de conceptos con la ciudad, rejilla de dos conceptos por línea, ritmo vertical unificado, fechas compactas con gris tenue, importes transparentes, formato numérico de montos, eliminación del total por trayecto y reducción vertical del header global, solicitado explícitamente por el product owner
...
- Formulario expandido del trayecto: las fechas pasan a dos campos paralelos compactos sin etiqueta intermedia. La fila de fechas reutiliza exactamente las dos columnas y el `column-gap` de los conceptos; cada campo empieza en la guía de su columna y termina en la guía donde comienza el importe fijo de esa misma columna. Así la fecha fin queda alineada con `Avión`, `Bus` y `Atracciones`. Los seis conceptos fijos de gasto se muestran en una rejilla de dos columnas, en este orden: `Hospedaje / Avión`, `Tren / Bus`, `Taxi / Atracciones`. `Atracciones` forma parte del mismo grupo fijo para compartir exactamente la misma geometría y separación que los otros conceptos.
...
- Campos de captura del trayecto: las fechas conservan fondo gris tenue `#f5f6f7`, sin borde pesado ni sombra. Los campos de importe vuelven a fondo transparente, también en focus, conservando texto gris oscuro `#4b5563` y el símbolo monetario próximo a la cifra.
