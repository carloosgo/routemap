...
test('expanded expense editor keeps compact fields and one exact vertical rhythm', async () => {
  ...
  assert.match(expenses, /--expense-amount-width:\s*74px/);
  assert.match(expenses, /--expense-date-bg:\s*#f5f6f7/);
  ...
  assert.match(expenses, /\.calendar-date\s*\{[\s\S]*width:\s*calc\(100% - var\(--expense-amount-width\)\)[\s\S]*height:\s*38px;/);
  ...
  assert.match(expenses, /\.moneycard__amount\s*\{[\s\S]*background:\s*transparent/);
