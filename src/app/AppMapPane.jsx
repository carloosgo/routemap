    ...
    <section className="mappane" aria-label={t('mapRegion')}>
      ...
      {openNoteSegmentId && (()=>{
        ...
        return <div className="segnote" data-segment-id={segment.id} role="dialog" aria-label={t('segmentNote')} style={{zIndex:720}}>
          ...
          <div className="segnote__foot"><span className="segnote__saved" data-persistence-state={persistenceState}>{persistenceHasCheck&&<IconCheck size={12} aria-hidden="true" />} {persistenceLabel}</span><span className="segnote__count">{(segment.note||'').length} / 500</span></div>
        </div>;
      })()}
