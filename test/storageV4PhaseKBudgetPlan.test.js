...
test('budget plan valida thresholds, nombre y monto', () => {
  assert.notEqual(run(['--amount=0']).status, 0);
  assert.notEqual(run(['--amount=-1']).status, 0);
  assert.notEqual(run(['--amount=10', '--thresholds=0,1']).status, 0);
  assert.notEqual(run(['--amount=10', '--thresholds=2.1']).status, 0);
  assert.notEqual(run(['--amount=10', `--display-name=${'x'.repeat(61)}`]).status, 0);

  const result = run([
    '--amount=10.000000001',
    '--thresholds=1,0.5,0.8,0.5',
