...
- Pulido visual de conceptos y montos: las etiquetas de concepto pasan de 13 px a 15 px y los SVG conservan su escala de 15 px; la caja horizontal del icono se mantiene en 18 px y el gap icono/texto en 4 px. Los importes pasan de 12 px a 14 px y el prefijo de moneda de 11 px a 13 px. `Otros gastos` adopta igualmente 15 px y conserva el gris subordinado `#8a929d`. El borde visible del icono de la primera columna comienza en la misma guía que el inicio del nombre de ciudad, sin inset extra del body. Todas las filas usan el mismo `row-gap` de escritorio, incluido `Atracciones`, los gastos dinámicos y `Otros gastos`; el espacio entre bloques se aplica una sola vez en el contenedor principal y las secciones dinámicas no añaden `padding-top` propio.
...
- Nota del origen: el botón de nota abre un editor dentro del bloque expandido. La nota se guarda como `originDetails.note`, limitada a 500 caracteres y validada por Rules v3/v4 como campo opcional para mantener compatibilidad con viajes anteriores. No crea colección, writer, timer ni ruta de persistencia paralela.
...
- El origen expandible reutiliza `ExpenseEditor` y el flujo de autosave/sync ya existente. La edición de `originDetails`, incluida su nota, se coalesce en la misma intención de metadata del root v4; no crea una colección, writer o temporizador de persistencia paralelo.
