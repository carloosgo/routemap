...
test('desktop itinerary geometry stays contained while detail forms remain in the note-style map modal', async () => {
  ...
  assert.doesNotMatch(compact, /workspace-panel:has\(\.editor-module--itinerary\)|itinerary-stop__nights|background:\s*var\(--atlas-accent\)/);
  assert.match(floating, /\.workspace-panel\s*\{[^}]*top:\s*calc\(var\(--trip-header-height\) \+ 14px\);[^}]*bottom:\s*14px;/s);
  assert.match(segmentForm, /formatSegmentDate\(segment\.startDate, locale\)/);
  assert.match(segmentForm, /formatSegmentDate\(segment\.endDate, locale\)/);
  assert.doesNotMatch(segmentForm, /formatSegmentNights|CollapsibleRegion|<SegmentBody|expanded=|onToggle=/);
  ...
  assert.match(header, /itinerary-stop__dates[\s\S]*segment__note-btn[\s\S]*segment__toggle segment__details-btn[\s\S]*removeSegment/s);
  assert.match(origin, /itinerary-stop__dates[\s\S]*segment__note-btn[\s\S]*segment__toggle segment__details-btn[\s\S]*itinerary-origin__clear/s);
