...
test('desktop itinerary uses tighter insets while the primary workspace is integrated', async () => {
  ...
  assert.match(compact, /\.itinerary-stop__date-range\s*\{[^}]*width:\s*60px;[^}]*min-width:\s*60px;[^}]*max-width:\s*60px;[^}]*flex-direction:\s*column;[^}]*align-items:\s*flex-end;[^}]*text-align:\s*right;/s);
