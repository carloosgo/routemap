...
- Alineación responsive del itinerario: en escritorio la columna de ciudad absorbe el ancho disponible, mientras fechas, noches, costo y acciones conservan su geometría compacta. La última acción de cada trayecto y el contenido principal comparten la misma guía derecha, evitando espacio blanco sobrante en pantallas amplias. El contenedor desplazable reserva de forma estable el gutter de la barra vertical, por lo que abrir/cerrar un trayecto no cambia el ancho útil de las filas cuando el contenido empieza o deja de requerir scrollbar.
...
- Ciudad origen: recupera exactamente la jerarquía visual original del nodo inicial de la timeline: nombre a 12.5 px, peso 500 y color `#5f6875`; país a 10.5 px, peso 400 y color `#9299a5`. En escritorio ocupa la misma guía de ciudad que cualquier parada y añade, en las posiciones equivalentes, fecha resumida, noches, costo, nota, expandir/contraer y cerrar. El botón cerrar limpia únicamente la selección de ciudad y no borra silenciosamente gastos, fecha o nota ya capturados.
...
- Nota del origen: el botón de nota abre la misma superficie flotante `.segnote` sobre el mapa que utilizan las notas de los demás trayectos, con el mismo textarea, contador, estado de persistencia y cierre; no se inserta dentro del bloque de fecha/gastos. La nota se guarda como `originDetails.note`, limitada a 500 caracteres y validada por Rules v3/v4 como campo opcional para mantener compatibilidad con viajes anteriores. No crea colección, writer, timer ni ruta de persistencia paralela.
...
- El header reutiliza cálculos ya disponibles (`total` y `breakdown`) y memoriza el resumen derivado del viaje; compactar su altura o ajustar localmente su tipografía no crea solicitudes ni cálculos nuevos.
