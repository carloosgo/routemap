...
export const V4_SAVE_CONFLICT_CODE = 'trip/save-conflict';
export const V4_SYNC_PENDING_CODE = 'trip/v4-sync-pending';
export const V4_SYNC_BUSY_CODE = 'trip/v4-sync-busy';
export const V4_WRITE_NOT_READY_CODE = 'trip/v4-write-not-ready';
...
function writeNotReadyError(message) {
  return codedError(V4_WRITE_NOT_READY_CODE, message);
}
...
function createContextId() {
  if (typeof globalThis.crypto?.randomUUID !== 'function') {
    throw new Error('crypto.randomUUID() es obligatorio para Storage v4.');
  }
  return `atlas-v4-${globalThis.crypto.randomUUID()}`;
}
...
export function createFirestoreV4TripWriter({
  db,
  uid,
  ...
  if (!db) throw new TypeError('Se requiere Firestore para Storage v4.');
  ...
  if (!local || !coordinator || !runtime) {
    throw new TypeError('La composición v4 está incompleta.');
  }
  ...
  async function acceptRemoteStateOnce({ tripId, remoteRoot, remoteCollections = {} } = {}) {
    if (closed) throw new Error('El writer v4 está cerrado.');
  ...
  async function removeOnce(tripId) {
    if (closed) throw new Error('El writer v4 está cerrado.');
    if (!lifecycleReady) {
      throw writeNotReadyError('Lifecycle v4 todavía no está habilitado.');
    }
    const id = requiredText(tripId, 'tripId');
