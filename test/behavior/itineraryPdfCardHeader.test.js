...
test('cards de notas mantienen número grande y un único eje visual para bandera ciudad y fecha', () => {
  assert.match(source, /page\.circle\(markerX, headerCenterY, 7\.2/);
  assert.match(source, /size: Number\(item\.number\) >= 10 \? 5\.9 : 6\.6/);
  assert.match(source, /drawCountryFlag\(page, item\.countryCode, box\.x \+ 25, headerCenterY - 5\.2, 15\.5, 10\.4\)/);
  assert.match(source, /const titleY = headerCenterY - \(layout\.titleSize \* 0\.46\)/);
  assert.match(source, /const inlineDateY = headerCenterY - \(layout\.dateSize \* 0\.46\)/);
});
