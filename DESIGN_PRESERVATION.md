...
- Pulido visual de conceptos y montos: las etiquetas de concepto conservan la escala ampliada de 13 px y los SVG la escala de 15 px; `Otros gastos` comparte exactamente la misma jerarquía tipográfica/iconográfica. El borde visible del icono de la primera columna comienza en la misma guía que el inicio del nombre de ciudad, sin inset extra del body. Todas las filas usan el mismo `row-gap`, incluido `Atracciones`, los gastos dinámicos y `Otros gastos`. Los importes y el botón `Otros gastos` usan gris oscuro `#4b5563`. El símbolo monetario permanece próximo a la cifra sin alterar el valor persistido.
- Colores de transporte solicitados: `Tren` usa rojo, `Avión` morado y `Taxi / Uber` rosa. `Bus`, `Hospedaje` y `Atracciones` conservan su codificación actual.
- Estabilidad de fechas: los triggers de fecha conservan exactamente la misma altura, padding, line-height y posición en reposo, hover, focus y estado abierto. No dibujan borde, fondo ni anillo de foco, y el botón de limpiar permanece absoluto sin modificar geometría; al pasar el mouse una fecha seleccionada no debe desplazarse visualmente.
- Otros gastos del trayecto: el botón `+ Otros gastos` vive al final del bloque. Cada clic conserva el mismo modelo canónico de `expenses.others`; las filas creadas se acomodan en la misma rejilla de dos columnas, dos gastos por línea, y el botón permanece alineado a la misma guía izquierda y al mismo paso vertical de los conceptos fijos.
