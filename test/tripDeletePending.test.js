...
test('el diálogo de borrado bloquea duplicados y advierte que la eliminación es definitiva', async () => {
  ...
  assert.match(dialog, /t\('deletePermanentWarning'\)/);
  assert.match(dialog, /isDeleting \? t\('deletingTrip'\) : t\('delete'\)/);

  assert.match(messages, /deletingTrip: 'Eliminando…'/);
  assert.match(messages, /deletePermanentWarning: 'Esta acción es definitiva y no se puede deshacer\.'/);
  assert.match(messages, /deletingTrip: 'Deleting…'/);
  assert.match(messages, /deletePermanentWarning: 'This action is permanent and cannot be undone\.'/);
});
