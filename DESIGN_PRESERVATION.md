...
- El origen reserva 53 px antes de la bandera (`14 + 10 + 19 + 10`) para que la bandera y la ciudad permanezcan exactamente alineados con los destinos aunque no muestre drag ni número.
...
## Ajuste solicitado: exportación PDF del Itinerario

- En escritorio, mientras `Itinerario` está activo, se añade un tercer botón circular azul debajo de Guardar y del menú de tres puntos. Reutiliza diámetro de 38 px, color `#18a9ca`, iconografía blanca, sombra y ritmo vertical de 8 px de esos controles del mapa.
- El control de exportación no se muestra en `Mis Rutas` ni en móvil durante esta primera versión; no añade estado persistido ni una opción inactiva de “próximamente”.
- La exportación reutiliza el mapa vivo de Itinerario: países coloreados, ruta punteada, marcadores numerados y encuadre pertenecen al render actual de Atlas. No se construye un mapa paralelo ni se sustituyen proveedores.
- Durante la preparación del PDF se ocultan temporalmente header, panel editor y controles interactivos y se redimensiona el mapa a una superficie A4 horizontal estable. Al cerrar el diálogo de impresión, el layout normal se restaura.
- La primera página contiene nombre del viaje, rango global de fechas, total del viaje, número de ciudades y países, noches, mapa y una leyenda compacta que conserva la misma numeración de los trayectos.
- Las páginas posteriores contienen la nota del origen y una tarjeta por cada parada en orden cronológico. Las ciudades repetidas permanecen como visitas independientes; los saltos de línea de las notas se conservan y una visita sin nota se representa de forma explícita sin eliminarla.
- Los valores de resumen provienen de `tripSummary(trip)` y `tripTotal(trip)`; las notas provienen exclusivamente de `originDetails.note` y `segment.note`. No se duplican cálculos de dominio ni se añade persistencia.
- Esta V1 usa el diálogo nativo de impresión/Guardar como PDF del navegador. No añade dependencias PDF, Functions, Storage v4, Firestore Rules ni solicitudes externas adicionales.

## Ajuste solicitado: cards compactas del Itinerario

- La vista predeterminada del panel integrado usa una sola card por fila y cada card aprovecha el ancho disponible del panel; el usuario puede cambiar explícitamente a una cuadrícula de dos cards por fila mediante el control de vista del propio panel.
- `Agregar ciudad` permanece siempre arriba de las cards. Al pulsarlo se abre el `CityAutocomplete` existente; no se crea un trayecto/card vacío al abrir el control. La nueva card aparece únicamente después de que el usuario selecciona una ciudad válida. Si existe el segmento placeholder inicial completamente vacío se reutiliza; en otro caso se agrega un nuevo segmento inicializado con esa ciudad.
- No se agrega dentro del panel un segundo bloque con título `Itinerario`, contador de ciudades o rango de fechas: esa información permanece únicamente donde ya existe en la interfaz global.
- Cada card conserva número de secuencia, ciudad, fecha/rango y costo, además de los mismos handlers para Nota, Detalles/conceptos y Eliminar.
- `Ciudad origen` se presenta como una card de primera clase dentro de la misma retícula; en vista de una columna ocupa el ancho disponible y en cuadrícula participa como una card más. Conserva su semántica de origen, su nota, detalles, costo y acción de limpiar ciudad.
- El visual de cada card es un SVG local y determinista. Para ciudades conocidas usa siluetas/arquitectura representativa de la ciudad (por ejemplo París, Ámsterdam, Brujas, Gante, Bruselas, Colonia, Berlín, Múnich, Núremberg, Bamberg, Rothenburg, Londres, Madrid y otras); las ciudades no reconocidas usan un fallback urbano genérico. No se descargan fotos ni se consulta un proveedor de imágenes.
- En la cuadrícula de dos columnas se reducen de forma específica la altura del visual, tipografía y cajas de acciones, manteniendo tracks reservados para fecha/costo/acciones para impedir solapamientos o saltos de geometría.
- En escritorio con `hover: hover` y `pointer: fine`, Nota, Detalles/conceptos y Eliminar permanecen visualmente ocultos hasta que la card recibe hover o `focus-within`; sus columnas siguen reservadas para que la card no cambie de geometría al aparecer los controles. La misma regla aplica a la card de origen.
- En dispositivos táctiles los tres controles permanecen visibles para no hacer depender una acción crítica de un estado hover inexistente.
- La selección de una ciudad al agregar sólo amplía el contrato existente de `appendSegment/addSegment` con un payload inicial `destination`; no añade un modelo paralelo, un writer nuevo ni una ruta nueva de persistencia.
- La card usa flujo normal para `visual → ciudad → footer`; `ItineraryCardLayoutFix.css` se carga después de las hojas legacy y fuerza altura natural en destino y origen, evitando que las geometrías históricas de 40/52 px provoquen texto duplicado, superposición entre cards o elementos montados. En la cuadrícula el footer separa métricas y acciones en filas distintas para proteger nombres largos, fechas y costos.
- El cambio no introduce fotografías, provider requests adicionales, campos persistidos nuevos, cambios de Storage v4, Firestore Rules, autosave, mapa ni modelo de dominio fuera de esa inicialización atómica del segmento.

## Ajuste solicitado: refinamiento tipográfico y cuadrícula predeterminada de cards

- Este apartado sustituye únicamente las reglas conflictivas del apartado anterior de cards compactas; el resto del contrato permanece vigente.
- La vista predeterminada pasa a ser la cuadrícula de dos cards por fila. El usuario conserva el control para cambiar a lista de una card por fila.
- El nombre de ciudad aumenta 2 px respecto a cada layout anterior: 15 px en lista y 14 px en cuadrícula.
- La fecha aumenta 2 px: 13 px en lista y 12 px en cuadrícula. El rango se presenta completo, con mes largo y en una sola línea, por ejemplo `24 Noviembre - 29 Noviembre`.
- La cantidad aumenta 2 px: 14 px en lista y 13 px en cuadrícula.
- En cuadrícula, fecha y cantidad usan filas separadas dentro del bloque de métricas para que meses largos e importes no se monten ni compitan por ancho.
