...
- La lista de destinos del panel integrado se proyecta como una cuadrícula de dos cards por fila en escritorio, reutilizando cada `SegmentForm` existente; no se crea un segundo modelo de itinerario ni una colección paralela.
...
- Cada card incorpora un visual genérico local de viaje: una pequeña ilustración lineal de arquitectura/ciudad con fondo tenue, coloreada a partir del color de secuencia. Es decorativa, no intenta identificar un monumento real y no depende de imágenes, Google Places, Street View ni otro proveedor.
- La bandera se conserva como referencia del país y se integra visualmente sobre la esquina inferior del visual genérico; el número de secuencia se mantiene superpuesto en la esquina superior.
- `Ciudad origen` conserva una única fila a ancho completo encima de la cuadrícula y no se convierte en una card numerada.
- En escritorio con `hover: hover` y `pointer: fine`, Nota, Detalles/conceptos y Eliminar permanecen visualmente ocultos hasta que la card recibe hover o `focus-within`; sus columnas siguen reservadas para que la card no cambie de geometría al aparecer los controles.
...
- El cambio no introduce fotografías, búsquedas adicionales, provider requests, campos persistidos, writers, cambios de Storage v4, Firestore Rules, autosave, mapa ni modelo de dominio.
