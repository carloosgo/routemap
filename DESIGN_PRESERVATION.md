...
- Cada destino numerado reserva el mismo slot total de 53 px para `número + bandera`: círculo de 19 px, gap de 4 px y bandera de 30 px. El círculo queda a la izquierda de la bandera; el origen y el retorno terminal al origen permanecen sin número y conservan la bandera alineada con las banderas numeradas.
...
- En reordenamientos, los SVG de ruta se reconcilian sobre los mismos nodos existentes y reciben su nueva geometría en el siguiente frame; nunca se vacía primero la capa para volverla a crear después.
- Los `AdvancedMarkerElement` de itinerario mantienen el frame anterior visible hasta que el frame con los marcadores nuevos está preparado, evitando un intervalo visual sin números durante el cambio de orden.
