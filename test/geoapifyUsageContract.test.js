...
test('el backend de ciudades consulta Geocoding Search directamente y mantiene límites mundiales sin catálogo canónico', async () => {
  ...
  assert.match(cityFunctions, /cached\(\s*'citySearchCache'/);
  assert.match(cityFunctions, /city:live:v1:/);
  assert.match(cityFunctions, /source: cachedProvider\.cacheHit \? 'provider-cache' : 'provider'/);
  assert.match(cityFunctions, /QUOTAS\.cityAutocomplete/);
  assert.doesNotMatch(cityFunctions, /readCityCatalogQuery|persistCityCatalogQuery|cityCatalog/);
