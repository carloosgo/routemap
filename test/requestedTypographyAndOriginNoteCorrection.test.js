...
test('desktop itinerary uses tighter insets while the primary workspace is integrated', async () => {
  ...
  assert.match(compact, /\.itinerary-stop__date-range\s*\{[^}]*width:\s*var\(--itinerary-date-width\);[^}]*min-width:\s*var\(--itinerary-date-width\);[^}]*max-width:\s*var\(--itinerary-date-width\);[^}]*flex-direction:\s*row;/s);
